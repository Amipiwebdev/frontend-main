 ...
 *
 * 2) Login popup input pe tap/click se close na ho (already handled) — keep same.
 */
...
const Header = () => {
  ...
  // ✅ Login popup close on outside click + ESC
  useEffect(() => {
    if (!showLogin) return;
  ...
  // ✅ Search close on outside click + ESC
  useEffect(() => {
    ...
    <header className="main-header">
      <nav className="navbar navbar-expand-lg abg-secondary p-0">
        <div className="container-fluid">
          ...
          {/* ✅ MOBILE ICONS — only render when !isDesktop (so searchRef never duplicates) */}
          {!isDesktop && (
