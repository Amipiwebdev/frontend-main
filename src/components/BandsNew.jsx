...
function getCartCheckState(payload) {
  ...
  const added = payload.addedtocart ?? payload.added_to_cart ?? payload.addedToCart;
...
function RingSizeSelect2({ options, value, onChange, loading, disabled }) {
  ...
  return (
    <>
      <style>{`
        .bands-page .ring-select2 .react-select__control{
          min-height:40px;
      ...
      <Select
        ...
        styles={{
          ...
          option: (base, state) => ({
            ...base,
            backgroundColor: state.isSelected ? BRAND : state.isFocused ? BRAND : "white",
            color: state.isSelected || state.isFocused ? "white" : "#223052",
            cursor: "pointer",
          }),
          menu: (base) => ({ ...base, zIndex: 100000 }),
          singleValue: (base) => ({ ...base, color: "#223052", fontWeight: 700 }),
          dropdownIndicator: (base) => ({ ...base, color: BRAND, "&:hover": { color: BRAND } }),
          indicatorSeparator: (base) => ({ ...base, backgroundColor: "#d7dbe6" }),
        }}
      />
    </>
  );
          ...
          <>
            <button className="lb-arrow lb-prev" aria-label="Previous" onClick={onPrev}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                <path d="M320 576C461.4 576 576 461.4 576 320C576 178.6 461.4 64 320 64C178.6 64 64 178.6 64 320C64 461.4 178.6 576 320 576zM199 303L279 223C288.4 213.6 303.6 213.6 312.9 223C322.2 232.4 322.3 247.6 312.9 256.9L273.9 295.9L424 295.9C437.3 295.9 448 306.6 448 319.9C448 333.2 437.3 343.9 424 343.9L273.9 343.9L312.9 382.9C322.3 392.3 322.3 407.5 312.9 416.8C303.5 426.1 288.3 426.2 279 416.8L199 336.8C189.6 327.4 189.6 312.2 199 302.9z"></path>
              </svg>
            </button>
...
/* ------------------------------------------------------------------ */
/*     Accordion Shell (NOW: works on Desktop + Mobile both)           */
/* ------------------------------------------------------------------ */

function AccordionShell({ id, title, openId, setOpenId, selectedLabel, children }) {
  const open = openId === id;

  return (
    <>
      <section className="bands-acc-card" id={`acc-${id}`}>
        ...
        >
          <div className="bands-acc-body-inner">{children}</div>
        </div>
      </section>
    </>
  );
...
const BandsNew = React.memo(function BandsNew() {
  ...
  const { user: authUser, redirectToLogin, refresh: refreshAuth } = useAuthState();
  ...
  /* ✅ Accordion state (NOW for desktop + mobile) */
  const [openId, setOpenId] = useState("stoneType");
  ...
  useEffect(() => {
    ...
    api.get(`/shapesnew`, { params: { stoneType: selected.stoneType, design: selected.design } }).then((res) => {
      if (!isActive) return;
  ...
  const openLightbox = useCallback((i) => setLbIndex((prev) => (prev === i ? prev : i)), []);
  const closeLightbox = useCallback(() => setLbIndex((prev) => (prev === -1 ? prev : -1)), []);
  const prevLightbox = useCallback(() => {
    setLbIndex((i) => (!galleryImages.length ? i : i <= 0 ? galleryImages.length - 1 : i - 1));
  }, [galleryImages.length]);
  const nextLightbox = useCallback(() => {
    setLbIndex((i) => (!galleryImages.length ? i : (i + 1) % Math.max(galleryImages.length, 1)));
  }, [galleryImages.length]);
  ...
  const selectedLabels = useMemo(() => {
    ...
    return {
      ...
      diamondSize:
        selected.diamondSize != null ? `${formatDiamondSize(selected.diamondSize)} ${sizeUnit.toUpperCase()}` : "",
  ...
  const handleFilterChange = useCallback(
    (key, value) => {
      ...
      // ✅ open next accordion automatically (nice UX)
      const currentIdx = FILTER_ORDER.indexOf(key);
      const nextKey = FILTER_ORDER[currentIdx + 1];
      if (nextKey) setOpenId(nextKey);
    },
    [data, ringOptions]
  );
  ...
  const computedPrice = useMemo(() => {
    const base =
      estPrice !== null ? Number(estPrice) : Number(getSelectedProductPrice(product, jewelryPriceLevel) ?? NaN);
  ...
  const displayPrice = useMemo(() => {
    ...
    return { hasTariff: false, tariffPer, regularConverted, withTariffConverted: null };
  ...
  return (
    <div className="bands-page">
      ...
      <style>{`
        ...
        @media(max-width: 991px){
          .bands-page .bands-right{position:static}
        }
        ...
        .bands-page .spinner{
          ...
          border:3px solid #d7dbe6;border-top-color:#2c3b5c;
          animation:bandsSpin .8s linear infinite;
        }
        @keyframes bandsSpin { to { transform: rotate(360deg);} }

        /* ✅ Product card spacing */
        .bands-page .product-stack{
          margin-top:8px;
          display:flex;
          flex-direction:column;
          gap:12px;
        ...
        <div className="row">
          ...
          <div className="col-12">
            <div className="bands-layout">
              {/* LEFT = GALLERY */}
              <div className="bands-left">
                <div className="bands-left-inner">
              ...
              {/* RIGHT = FILTER ACCORDIONS + PRODUCT INFO BELOW */}
              <div className="bands-right">
                {/* Stone Type */}
                <AccordionShell
                  id="stoneType"
                  title="STONE TYPE"
                  ...
                  <div className="filter-options">
                    {data.settingStyles.map((sc) => (
                      <button
                        ...
                        type="button"
                        className={
                          "filter-card" + (selected.settingStyle === (sc.psc_id || sc.id) ? " selected" : "")
                        }
                ...
                {/* ✅ PRODUCT INFO BELOW FILTERS */}
                <div className="product-stack">
                  {/* YOUR SELECTION / DETAILS (same as before, no logic change) */}
                    ...
                    {product ? (
                      <div className="detail">
                        <div className="box-grey table-responsive">
                          {/* --- keep your existing selection card markup here ---
                              You already have it in your old file.
                              If you want, paste the same block exactly here.
                              (I’m keeping it minimal here to avoid breaking your structure.)
                          */}
                          {/* ✅ Keep your existing "YOUR SELECTION" block EXACTLY as it was */}
                          {/* -------- START: paste your existing selection-card block here -------- */}
                          {/* (You can copy-paste from your current file: <main className="wrap"> ... </main>) */}
                          {/* -------- END -------- */}
                          {/* NOTE: If you want I can paste that big block also, but it will be very long. */}
                          <main className="wrap">
                  ...
                  {/* RIGHT SIDE ACTIONS / DESCRIPTION */}
                  <div className={`stable-wrap ${productLoading ? "is-loading" : ""}`}>
                    {productLoading && (
                      <div className="block-overlay">
                        <div className="spinner" />
...
/* ✅ LoginModal unchanged from your file */
